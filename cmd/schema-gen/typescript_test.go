...
func TestGeneratedTypeScriptFilesFullyAccounted(t *testing.T) {
	...
	packageRoot := filepath.Join(root, "typescript")
	var expected []string
	for _, path := range typeScriptGeneratedFiles(root, packageRoot) {
	...
	var actual []string
	for _, generatedRoot := range []string{filepath.Join(packageRoot, "src"), filepath.Join(packageRoot, "tests")} {
		...
		if err != nil {
			t.Fatalf("walk generated TypeScript files under %s: %v", generatedRoot, err)
		}
	}
