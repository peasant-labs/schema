...
`rcN` tags publish **prereleases**. A final `vX.Y.Z` does not require a preceding
rc. It still runs the release gates and, when a prior release tag exists, the
compatibility diff against that prior release.
