...
func requireDurableFixtures(t *testing.T) durableGraphFixtures {
	t.Helper()
	fx, err := loadDurableGraphFixtures()
	if err != nil {
		t.Fatal(err)
	}
...
func projectionPair(t *testing.T, input durableMirrorInput) (schema.SessionDetailPayload, schema.UnifiedMetadata) {
	t.Helper()
