...
- `*/invalid/metadata.json` — a `source.format` **enum violation** (`xml`/`yaml`/`csv` ∉ {jsonl,json}) that otherwise passes the handler field checks. Drives the village **OpenAPI enforce → 422** path (invalid entry).
...
## Distinct floors (do NOT conflate)
...
from the **push-acceptance** floor (negotiation, gates incoming uploads) and
